...
export default function WeaponList({ weaponList, handleSelectWeapon, handleTransition }) {
    return (
        weaponList && weaponList.length > 0 ? (
            ...
            >
                {weaponList.map((item, index) => (
                    <div
                        ...
                        onClick={() => handleTransition(() => handleSelectWeapon(item.name))}
                        style={{ height: "auto" }}
                    >
                        <div className="w-16 h-16 flex items-center justify-center mb-1">
                            <img
                                src={`/icons/weapon/${item.name.toLowerCase()}.png`}
            ...
            <p className="text-gray-400">무기 목록을 찾을 수 없습니다.</p>
        )
    );
}
